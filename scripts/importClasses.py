...
# Parse the classData string
classData_dict = json.loads(classData)

# Firebase Admin SDK setup (requires service account key JSON file)
cred = credentials.Certificate('../attendance-app-ss-firebase-adminsdk-fbsvc-06fcdb7f42.json')  # <-- Replace with your service account key path
firebase_admin.initialize_app(cred)
db = firestore.client()

# Upload each class to Firestore
for class_name, class_info in classData_dict.items():
    db.collection('classes').document(class_name).set(class_info)

print('Class data uploaded to Firestore.')